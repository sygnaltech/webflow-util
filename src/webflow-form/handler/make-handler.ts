...
export class WfuFormHandlerMake extends WfuFormHandler {
    ...
    handleResponse(data, status, xhr) {
        ...
        if (xhr.status >= 200 && xhr.status < 300) { 
            super.handler.setWebflowFormMode(WebflowFormMode.Success, xhr.responseJSON?.message);
        } else {
            super.handler.setWebflowFormMode(WebflowFormMode.Error, xhr.responseJSON?.message);
        }
    ...
    handleFailResponse(jqxhr, settings, ex) {
        ...
        if (jqxhr.status == 400) {
            ...
            super.handler.setWebflowFormMode(WebflowFormMode.Error, jqxhr.responseJSON?.message);
        } else {
            super.handler.setWebflowFormMode(WebflowFormMode.Error, jqxhr.responseJSON?.message);
        }
