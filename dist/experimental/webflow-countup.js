(() => {
    ...
    debug(...args) {
      if (this.enabled) {
        ...
        for (let i2 = 0; i2 < args.length; i2++) {
          if (typeof args[i2] === "string" && args[i2].includes("%c") && typeof args[i2 + 1] === "string") {
            formattedMessage += args[i2] + " ";
            styles.push(args[i2 + 1]);
            i2++;
          } else {
            formattedMessage += "%c" + args[i2] + " ";
            styles.push("");
          }
        }
  ...
  var Sa5CountUp = class {
    ...
    startCountups(group) {
      ...
      const that = this;
      const elements = document.querySelectorAll(`[${"wfu-countup" /* ATTR_COUNTUP */}='${group}']`);
      elements.forEach((element) => {
    ...
    installCountupWaypoints() {
      ...
      const triggers = document.querySelectorAll(
        Sa5Attribute.getBracketed("wfu-countup-trigger" /* ATTR_COUNTUP_TRIGGER */)
      );
      triggers.forEach((trigger) => {
        const group = trigger.getAttribute(
          "wfu-countup-trigger" /* ATTR_COUNTUP_TRIGGER */
        );
        var countups = [];
        const elements = document.querySelectorAll(`[${"wfu-countup" /* ATTR_COUNTUP */}='${group}']`);
        elements.forEach((element) => {
      ...
      const counters = document.querySelectorAll(
        Sa5Attribute.getBracketed("wfu-countup" /* ATTR_COUNTUP */)
      );
      counters.forEach((c) => {
